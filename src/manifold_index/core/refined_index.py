...
try:
    from manifold_index.core._c_kernel._c_tet_index import (  # type: ignore[import-not-found]
...
def compute_refined_index(
    ...
    for term in terms:
        ...
        e_int: list[Fraction] = term["e_int"]

        # --- Extract hard-edge fugacity exponents (first k entries of e_int) ---
        # Each e_int entry is a Fraction object.
...
def compute_refined_index_batch(
    ...
    for m_ext, e_ext in entries:
        ...
        for term in terms:
            ...
            eta_exps_x2 = tuple(
                int(e_int[a] * 2) for a in range(k)
            )
