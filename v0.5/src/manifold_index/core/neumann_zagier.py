...
class NeumannZagierData:
    ...
    def g_NZ_inv(self) -> np.ndarray:
        ...
        # limit_denominator recovers the exact fraction from the float.
        # Use a large bound (10**9) to avoid accidentally rounding entries
        # whose true denominator exceeds 1000 (e.g. manifolds with large
        # Smith invariant factors).
        result = np.array(
            [[Fraction(v).limit_denominator(10**9) for v in row]
