...
def check_weyl_symmetry(
    ...
    Because the refined-index summation is truncated at *q_order_half*,
    the highest-order coefficients can be incomplete (truncation artifacts).
    When *q_order_half* is supplied, only terms whose q-half-power is
    strictly below a safe cutoff are compared.  The cutoff for a sector
    pair (m, e) / (−m, −e) is::

        q_cutoff = q_order_half − max(2, q_order_half // 3)

    This discards the boundary region where truncation makes the
    ...
    q_order_half : int or None
        The truncation order used when computing the refined index.
        If given, boundary terms are excluded from comparison.

    Returns
    -------
    dict mapping  (tuple(m_ext), tuple(e_ext))  →  bool (True = symmetric).
    """
    # Compute safe comparison cutoff
    if q_order_half is not None:
        q_cutoff = q_order_half - max(2, q_order_half // 3)
    else:
        q_cutoff = None  # compare everything
    ...
    for m_ext, e_ext, result in entries:
        ...
        for key, coeff in result.items():
            ...
            if q_cutoff is not None and key[0] > q_cutoff:
                continue
            new_key = (key[0],) + tuple(
...
def run_weyl_checks(
    ...
    if ab is not None:
        weyl_sym = check_weyl_symmetry(
            entries, num_hard, ab, q_order_half=q_order_half,
        )
    else:
