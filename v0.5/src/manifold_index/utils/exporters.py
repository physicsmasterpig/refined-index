...
def write_full_report(
    ...
    if dehn_results:
        from manifold_index.app.workers import (
            TransformedFillResult, MultiCuspFillResult, UnrefinedFillResult,
        )
        ...
        for res in dehn_results:
            fill_idx += 1
            if isinstance(res, UnrefinedFillResult):
                _append_unrefined_filling(
                    L, res, nz, n, r, fill_idx,
                )
            elif isinstance(res, TransformedFillResult):
...
def _append_unrefined_filling(
    ...
    """Append an unrefined Dehn filling section (fallback — no NC cycles)."""
    L.append(rf"\subsection{{Filling {fill_idx} (Unrefined): "
             rf"Cusp {res.cusp_idx}, slope $({res.P_user}, {res.Q_user})$}}")
    L.append("")
    L.append(r"\emph{No non-closable cycles found in the search range.  "
             r"Showing the unrefined Dehn filling index.}")
    L.append("")

    L.append(r"\subsubsection{Unrefined Filled Index}")
    for m_other, e_other, fr in res.fill_results:
        label = _charge_label(m_other, e_other, latex=True) if m_other else ""
        # Unrefined: series is {k: coeff} where k → q^{k/2}
        tex = _unrefined_series_to_latex(fr.series)

        if label:
            L.append(rf"\paragraph{{{label}}}")

        L.append(rf"Kernel terms: {fr.n_kernel_terms}.")
        L.append(r"\[")
        L.append(rf"  I_{{{res.P_user}/{res.Q_user}}} = {tex}")
        L.append(r"\]")
        L.append("")


def _unrefined_series_to_latex(series: dict) -> str:
    """Format an unrefined q-series ``{k: coeff}`` as LaTeX."""
    if not series:
        return "0"
    terms: list[str] = []
    for k in sorted(series):
        ...
        if c_frac.denominator == 1:
            ...
            c_str = rf"\frac{{{c_frac.numerator}}}{{{c_frac.denominator}}}"
        ...
        if k == 0:
            ...
            if c_str == "1":
                ...
                terms.append(rf"{c_str}\,q")
        elif k % 2 == 0:
            pw = k // 2
            if c_str == "1":
                terms.append(rf"q^{{{pw}}}")
            elif c_str == "-1":
                terms.append(rf"-q^{{{pw}}}")
            else:
                terms.append(rf"{c_str}\,q^{{{pw}}}")
        else:
            if c_str == "1":
                terms.append(rf"q^{{{k}/2}}")
            elif c_str == "-1":
                terms.append(rf"-q^{{{k}/2}}")
            else:
                terms.append(rf"{c_str}\,q^{{{k}/2}}")
    if not terms:
        return "0"
    result = terms[0]
    for t in terms[1:]:
        ...
        if isinstance(res, UnrefinedFillResult):
            ...
            for m_other, e_other, fr in res.fill_results:
                fills.append({
                    ...
                    "series": {str(k): int(v) if isinstance(v, (int, np.integer)) else str(v) for k, v in fr.series.items()},
                })
            out.append({
                "type": "unrefined",
        ...
        for res in dehn_results:
            if isinstance(res, UnrefinedFillResult):
                L.append(f'(* Cusp {res.cusp_idx}: slope ({res.P_user},{res.Q_user}), '
                         f'UNREFINED (no NC cycles found) *)')
                for m_other, e_other, fr in res.fill_results:
                    m_str = "{" + ", ".join(str(x) for x in m_other) + "}" if m_other else "{}"
                    e_str = "{" + ", ".join(str(x) for x in e_other) + "}" if e_other else "{}"
                    expr = _unrefined_to_mathematica(fr.series)
                    L.append(
                        f'IFilled["{name}", {{{res.P_user}, {res.Q_user}}}, '
                        f'{m_str}, {e_str}] = {expr};'
                    )
            elif isinstance(res, TransformedFillResult):
                L.append(f'(* Cusp {res.cusp_idx}: slope ({res.P_user},{res.Q_user}), '
...
def _unrefined_to_mathematica(series: dict) -> str:
    """Format an unrefined q-series ``{k: coeff}`` as a Mathematica expression."""
    if not series:
        return "0"
    terms: list[str] = []
    for k in sorted(series):
        ...
        if k == 0:
            ...
            if c_str == "1":
                ...
                terms.append(f"{c_str}*q^{pw}")
        else:
            if c_str == "1":
                terms.append(f"q^({k}/2)")
            elif c_str == "-1":
                terms.append(f"-q^({k}/2)")
            else:
                terms.append(f"{c_str}*q^({k}/2)")
    ...
    if include_dehn and dehn_results:
        ...
        for res in dehn_results:
            if isinstance(res, UnrefinedFillResult):
                for m_other, e_other, fr in res.fill_results:
                    tex = _unrefined_series_to_latex(fr.series)
                    lines.append(rf"I_{{{res.P_user}/{res.Q_user}}} = {tex}")
            elif isinstance(res, TransformedFillResult):
                for m_other, e_other, fr in res.fill_results:
                    tex = to_latex_filled_series(
                        fr.series, fr.num_hard, fr.num_cusp_eta
